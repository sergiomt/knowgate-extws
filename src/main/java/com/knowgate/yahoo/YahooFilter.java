...
public class YahooFilter implements Filter {
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        if (DebugFile.trace) DebugFile.writeln("oauth properties file: " + filename);
        ...
        if (DebugFile.trace) DebugFile.writeln("redirect if access token not found: " + redirect);
        ...
        if (DebugFile.trace) DebugFile.writeln("oauth client connection class: " + oauthConnectionClass);
        try {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (DebugFile.trace) DebugFile.writeln("sessionType: " + sessionType);

        OAuthAccessor accessor = new OAuthAccessor(consumer);

        if (sessionType == null) {
            if (redirect) {
                if (DebugFile.trace) DebugFile.writeln("redirecting user to yahoo acquire access token");
                redirectForAuthorization(accessor, request, response);
                return;
            } else {
                if (DebugFile.trace) DebugFile.writeln("inserting YahooSession suitable for 2-legged oauth calls into request attribute");
            ...
            if (consumer.consumerKey == null || !consumer.consumerKey.equals(getParam(request, "yap_consumer_key"))) {
            	if (DebugFile.trace) DebugFile.writeln("Consumer key from YAP does not match config.");
            ...
            try {
                ...
                method.validate(msg);
            	  ...
            	  if (DebugFile.trace) DebugFile.writeln("Signature from YAP failed." + ex.getMessage());
                clearSession(request, response);
                if (redirect) {
                    redirectForAuthorization(accessor, request, response);
                    return;
            ...
            try {
                ...
                if (consumer.consumerKey == null || !consumer.consumerKey.equals(accessToken.getConsumer())) {
                	if (DebugFile.trace) DebugFile.writeln("Consumer key for token does not match the defined Consumer Key.  The Consumer Key has probably changed since the user last authorized the application.");
                    clearSession(request, response);
                    if (redirect) {
                        redirectForAuthorization(accessor, request, response);
                        return;
                ...
                if (DebugFile.trace) DebugFile.writeln("AT Expires in: " + (accessToken.getTokenExpires() - now));
            ...
            try {
                ...
                if (DebugFile.trace) DebugFile.writeln("got oauth_verifier " + verifier);

                try {
                	if (DebugFile.trace) DebugFile.writeln("request token found, fetching access token for user");
    ...
    private AccessToken fetchAccessToken(OAuthAccessor accessor, RequestToken requestToken, String verifier) throws IOException, URISyntaxException, OAuthException {
        ...
        if (DebugFile.trace) DebugFile.writeln("setting access token expires in: " + at.getTokenExpires());
        if (DebugFile.trace) DebugFile.writeln("setting access token handle expires in: " + at.getHandleExpires());
        return at;
    }
    ...
    private void accessTokenExpired(OAuthAccessor accessor, HttpServletRequest request, HttpServletResponse response, AccessToken accessToken, FilterChain filterChain) throws IOException, ServletException, JSONException, OAuthException, URISyntaxException {
        if (DebugFile.trace) DebugFile.writeln("access token expired, attempting to renew");
        long now = System.currentTimeMillis() / 1000;
    ...
    public void clearSession(HttpServletRequest req, HttpServletResponse res) {        
        if (DebugFile.trace) DebugFile.writeln("clear session requested");
        Cookie at = new Cookie("yosdk_at", "");
        at.setMaxAge(0);
